...
const AMENDED_BOOK7_PAGES = [
  'ita/documents/cic_libroVII_1671-1673_it.html',
  'ita/documents/cic_libroVII_1674_it.html',
  'ita/documents/cic_libroVII_1675-1678_it.html',
  'ita/documents/cic_libroVII_1679-1682_it.html',
  'ita/documents/cic_libroVII_1683-1687_it.html',
  'ita/documents/cic_libroVII_1688-1690_it.html',
  'ita/documents/cic_libroVII_1691_it.html',
...
function discoverBook7Urls(indexHtml: string) {
  ...
  const discovered = [...paths, ...fallback].map((path) =>
    new URL(path.replace(/^\.\.\//, ''), BASE).href,
  )
  return [...new Set([...discovered, ...AMENDED_BOOK7_PAGES])]
}

function extractCanons(body: string, canonMap: Map<number, string>) {
  const matches = [...body.matchAll(/(?:^|\n|\s)Can\.\s*(\d+)(?:\^?\{?n\}?|n)?\s*-\s*/gi)]
  for (let i = 0; i < matches.length; i++) {
    ...
    value = value
      .split(/\n(?:CODICE DI DIRITTO CANONICO|LIBRO VII|PARTE |TITOLO |CAPITOLO |Articolo |\(.*Indica che il testo|Cf:)/i)[0]
      .trim()
...
async function main() {
  ...
  for (const url of unique) extractCanons(text(await get(url)), canonMap)

  const missing: number[] = []
  for (let number = 1400; number <= 1752; number++) if (!canonMap.has(number)) missing.push(number)
  ...
  await writeFile(
    MANIFEST,
    JSON.stringify({book: 7, range: [1400, 1752], expectedCanons: 353, language: 'it', source: 'Santa Sede', indexUrl: INDEX, retrievedAt: new Date().toISOString(), pages: unique.length, sha256}, null, 2) + '\n',
    'utf8',
  )
