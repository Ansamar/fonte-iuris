...
const HTML = resolve('scripts/fontes-canonical/data/de-concordia-inter-codices/official-it.html')
...
async function main(){
  console.log('DE CONCORDIA INTER CODICES — BACKFILL TESTO UFFICIALE')
  if(!existsSync(HTML)) throw new Error(`Snapshot ufficiale non trovato: ${HTML}`)
  const text=extract(readFileSync(HTML,'utf8'))
  if(text.length<5000) throw new Error(`Testo estratto troppo breve: ${text.length} caratteri`)
  const low=text.toLowerCase()
  for(const marker of ['de concordia inter codices','codice di diritto canonico']) if(!low.includes(marker)) throw new Error(`Marker atteso assente: ${marker}`)
  const docs=await client.fetch(`*[_type=='sourceDocument' && (lower(title) match '*concordia*' || lower(shortTitle) match '*concordia*')]{_id,title}`)
  if(docs.length!==1) throw new Error(`Atteso 1 sourceDocument De concordia, trovati ${docs.length}`)
  const id=docs[0]._id
  await client.patch(id).set({sourceText:text}).commit()
  const readback=await client.fetch(`*[_id==$id][0]{sourceText}`,{id})
  if(!readback?.sourceText || readback.sourceText.length<5000) throw new Error('Read-back fallito')
