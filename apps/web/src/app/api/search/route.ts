...
type ItalianRow={_id:string;provisionId:string;title:string;provisionType?:string;issuer?:string;status?:string;summary?:string;text?:string;effectiveFrom?:string;sourceId?:string;sourceTitle?:string;sourceText?:string;officialUrl?:string};
...
export async function GET(request:NextRequest){
 try{
  ...
  for(const row of italian){const needle=normalize(q),title=normalize(row.title),summary=normalize(row.summary),text=normalize(row.text||row.sourceText);let score=55;if(title===needle)score+=90;else if(title.includes(needle))score+=60;if(summary.includes(needle))score+=25;if(text.includes(needle))score+=18;results.push({_id:row.sourceId||row._id,resultType:"sourceDocument",label:"Atto",title:row.title,subtitle:[row.issuer,row.sourceTitle].filter(Boolean).join(" · "),snippet:excerpt(row.summary||row.text||row.sourceText,q),status:row.status,effectiveFrom:row.effectiveFrom,officialUrl:row.officialUrl,matchReason:"Fonte normativa",score});}
  ...
  return NextResponse.json({results:results.slice(0,60),counts:{canons:canons.length,acts:italian.length+(documents.length-linkedSourceIds.size),legalRelations:relations.length}});
