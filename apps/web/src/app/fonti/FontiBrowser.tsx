...
export default function FontiBrowser({sources}:{sources:Source[]}){
 ...
 const [mode,setMode]=useState<"start"|"search"|"browse">("start");
 ...
 const reset=()=>{setQuery("");setType("Tutte")};
 const startSearch=()=>{setMode("search");setTimeout(()=>document.getElementById("fonti-search")?.focus(),0)};

 return <>
  {mode==="start"&&<section aria-label="Come vuoi consultare le fonti" style={{margin:"0 0 24px"}}>
   <h2 style={{font:"700 1.35rem Georgia,serif",color:"var(--blue)",margin:"0 0 12px"}}>Come vuoi iniziare?</h2>
  ...
  {mode!=="start"&&<div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"baseline",margin:"0 0 12px"}}><div><p style={{fontSize:10,fontWeight:850,color:"var(--gold)",textTransform:"uppercase",margin:"0 0 4px"}}>{mode==="search"?"Ricerca diretta":"Percorso guidato · fonti"}</p><h2 style={{font:"700 1.35rem Georgia,serif",color:"var(--blue)",margin:0}}>{mode==="search"?"Trova l’atto che conosci":"Scegli il tipo di fonte"}</h2></div><button onClick={()=>{reset();setMode("start")}} style={{border:0,background:"transparent",color:"var(--blue)",fontWeight:800,cursor:"pointer"}}>Torna all’inizio</button></div>}

  {mode!=="start"&&<div style={{display:"grid",gridTemplateColumns:"minmax(260px,1fr) auto",gap:12,alignItems:"center",margin:"0 0 22px"}}>
  ...
  {mode!=="start"&&<><div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",margin:"0 0 12px",fontSize:12,color:"var(--muted)"}}><span>{filtered.length} {filtered.length===1?"atto":"atti"}</span>{(query||type!=="Tutte")&&<button onClick={reset} style={{border:0,background:"transparent",color:"var(--blue)",fontWeight:700,cursor:"pointer"}}>Azzera filtri</button>}</div>
